...
user.register = async (req, res) => {
  try {
    ...
    apiResponseHandler.sendResponse(200, true, user, (response) => {
      res.json(response);
    });
  } catch (error) {
    console.log(error);
...
user.update = async (req, res) => {
  try {
    //step 1: to get the id and email of the user
    const { id } = req.params;
...
user.delete = async (req, res) => {
  try {
    //step 1 taking id in the req. params
    const { id } = req.params;
...
user.dataInfoUser = async (req, res) => {
  try {
...
user.getOnlyUserData = async (req, res) => {
  try {
    //step 1: id in req.params
    const { id } = req.params;
    //step 2: making a db call for get all user
    const allUser = await userModel.find({ _id: id }).select("-password");
    //step 3: send data in the response
    if (allUser) {
      apiResponseHandler.sendResponse(200, true, allUser, (response) => {
        res.json(response);
      });
